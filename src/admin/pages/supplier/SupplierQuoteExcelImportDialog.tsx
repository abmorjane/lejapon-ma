    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !busy && onOpenChange(nextOpen)}>
      <DialogContent className="flex max-h-[92dvh] max-w-6xl flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="border-b px-6 py-4 pr-12">
          ...
          <DialogDescription>
            Prévisualisation de la première feuille uniquement. Les autres feuilles ne sont jamais analysées.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-lg border p-3">
                <p className="text-xs text-muted-foreground">Première feuille détectée</p>
              ...
              <div className="rounded-lg border p-3">
                <p className="text-xs text-muted-foreground">Lignes reconnues</p>
                <p className="mt-1 text-xl font-semibold">{preview.counts.recognized}</p>
              </div>
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-950">
                <p className="text-xs">Lignes à vérifier</p>
            ...
            <div className="flex flex-wrap gap-2">
              {(Object.keys(sectionLabels) as SupplierQuoteImportSection[]).map((section) => (
                <Badge key={section} variant="outline">
                  {sectionLabels[section]} {preview.counts.bySection[section]}
                </Badge>
              ))}
              <Badge variant="secondary">{preview.ignoredSheetCount} autre(s) feuille(s) ignorée(s)</Badge>
            </div>
            ...
            {preview.handling.detected && (
              <div className="space-y-3 rounded-lg border border-sky-200 bg-sky-50 p-4 text-sky-950">
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <div>
                    <p className="text-xs">Sous-total services avant handling (source)</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatJpy(preview.financialSummary.sourceServiceSubtotalJpy)}</p>
                  </div>
                  <div>
                    <p className="text-xs">Handling fournisseur : {formatPercentage(preview.financialSummary.supplierHandlingPercentage)}</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatJpy(preview.financialSummary.supplierHandlingJpy)}</p>
                  </div>
                  <div>
                    <p className="text-xs">Total fournisseur final (source)</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatJpy(preview.financialSummary.sourceFinalSupplierTotalJpy)}</p>
                  </div>
                  <div>
                    <p className="text-xs">Total fournisseur projeté</p>
                    <p className="mt-1 text-lg font-semibold tabular-nums">{formatJpy(preview.financialSummary.calculatedSupplierTotalJpy)}</p>
                  </div>
                </div>
                {preview.handling.suggestedCategories && <p className="text-sm">Périmètre suggéré par le montant source : {preview.handling.suggestedCategories.map(category=>HANDLING_CATEGORY_LABELS[category]).join(", ")}. Vérifiez les conditions retenues ci-dessous.</p>}
                <SupplierHandlingFields value={preview.handling} disabled={busy} onChange={terms=>onHandlingChange(terms)} />
                <p className="text-sm">{preview.handling.scopeOrigin==="inferred" ? "Périmètre suggéré par le montant source : à confirmer." : !preview.handling.confirmed ? "Handling scope: confirmation required" : "Périmètre confirmé par le fournisseur."}</p>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={preview.handling.confirmed} disabled={busy} onChange={event=>event.target.checked && onHandlingChange(preview.handling)} /> Je confirme le pourcentage et les catégories du handling.</label>
                {supplierQuoteImportHandlingDifference(preview)!==null && Math.abs(supplierQuoteImportHandlingDifference(preview)!)>0.01 && <div role="alert" className="space-y-2 text-sm">
                  <p>Écart du handling calculé avec le montant source : {formatJpy(supplierQuoteImportHandlingDifference(preview))}. Le montant source ne sera pas modifié.</p>
                  <label className="flex items-center gap-2"><input type="checkbox" checked={preview.handling.mismatchAcknowledged} disabled={busy} onChange={event=>onHandlingChange(preview.handling,event.target.checked)} /> J’accepte explicitement le handling recalculé et son écart avec la source.</label>
                </div>}
                {supplierQuoteImportHandlingErrors(preview).map(error=><p role="alert" className="text-sm" key={error}>{error}</p>)}
                <p className="text-xs">
                  Le handling est calculé au niveau du devis sur les catégories confirmées. Le montant source reste conservé dans l’audit.
                </p>
              </div>
            )}
              ...
              && Math.abs(preview.financialSummary.travelOsVarianceJpy) > 1 && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
                  <div className="flex items-center gap-2 font-medium">
                    ...
                    Réconciliation à vérifier avant confirmation
                  </div>
                  <p className="mt-1 text-xs">
                    Services recalculés {formatJpy(preview.financialSummary.calculatedServiceSubtotalJpy)} + handling {formatJpy(preview.financialSummary.supplierHandlingJpy)} = total fournisseur importé {formatJpy(preview.financialSummary.calculatedSupplierTotalJpy)}. Écart avec le total fournisseur Excel : {formatJpy(preview.financialSummary.travelOsVarianceJpy)}.
                  </p>
                </div>
              )}
            ...
            <div className={`rounded-lg border p-3 text-sm ${importMode === "new_version" ? "border-amber-200 bg-amber-50 text-amber-950" : "bg-muted/40"}`}>
              {importMode === "new_version"
                ? `Sécurité versioning : le devis courant reste intact. L’import créera V${nextVersionNumber} en brouillon.`
                : "Le brouillon courant ne contient aucune ligne enregistrée : l’import sera ajouté directement à cette version."}
            </div>
            ...
            {preview.counts.unresolved > 0 && (
              <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950">
                <div className="flex items-center gap-2 font-medium">
                  <AlertTriangle className="h-4 w-4" aria-hidden="true" />
                  {preview.counts.unresolved} ligne(s) nécessitent une décision
                </div>
                <p className="mt-1 text-xs">L’import reste bloqué tant que chaque sous-total Excel non renseigné n’a pas été traité explicitement.</p>
              </div>
            )}
            ...
            {preview.workbookWarnings.length > 0 && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
                <div className="flex items-center gap-2 font-medium">
                  <AlertTriangle className="h-4 w-4" aria-hidden="true" />
                  {preview.workbookWarnings.length} erreur(s) Excel neutralisée(s)
                </div>
                <p className="mt-1 text-xs">{preview.workbookWarnings.join(" ")}</p>
              </div>
            )}
            ...
            <div className="overflow-x-auto rounded-lg border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[110px]">Catégorie</TableHead>
                    <TableHead className="min-w-[260px]">Description</TableHead>
                    <TableHead className="min-w-[170px]">Dates</TableHead>
                    <TableHead className="text-right">Quantité</TableHead>
                    <TableHead className="text-right">Prix</TableHead>
                    <TableHead className="text-right">Sous-total Excel</TableHead>
                    <TableHead className="text-right">Calcul Travel OS</TableHead>
                    <TableHead className="text-right">Impact retenu</TableHead>
                    <TableHead className="min-w-[190px]">Décision</TableHead>
                    <TableHead className="min-w-[130px]">État</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {preview.rows.map((row) => (
                    <TableRow key={`${row.sourceRow}-${row.section}-${row.description}`}>
                      <TableCell className="py-3 text-xs">{row.kind==="supplier_handling" ? "Handling du devis" : sectionLabels[row.section]}</TableCell>
                      <TableCell className="py-3">
                        <p className="font-medium">{row.description}</p>
                        <p className="mt-0.5 text-xs text-muted-foreground">Ligne Excel {row.sourceRow}</p>
                        {row.warnings.length > 0 && <p className="mt-1 text-xs text-amber-700">{row.warnings.join(" ")}</p>}
                      </TableCell>
                      ...
                      <TableCell className="py-3 text-right tabular-nums">{row.excelSubtotalJpy === null ? "Non renseigné" : formatJpy(row.excelSubtotalJpy)}</TableCell>
                      ...
                      <TableCell className="py-3">
                        {row.requiresResolution ? (
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="include_recalculated">Inclure le recalcul</SelectItem>
                              <SelectItem value="import_uncharged">Importer non chiffré</SelectItem>
                              <SelectItem value="ignore">Ignorer la ligne</SelectItem>
                            </SelectContent>
                          </Select>
                        ) : "—"}
                      </TableCell>
                      <TableCell className="py-3">
                        <span className="inline-flex items-center gap-1.5 text-xs">
                          <StateIcon state={row.state} /> {stateLabel[row.state]}
                        </span>
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <p className="text-xs text-muted-foreground">
              Les lignes sans sous-total Excel exigent une décision. « Inclure le recalcul » retient le montant Travel OS, « Importer non chiffré » conserve la prestation avec un impact nul, et « Ignorer » ne crée aucune ligne.
            </p>
          </div>
        )}
