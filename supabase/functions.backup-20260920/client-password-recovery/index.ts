...
const GENERIC_MESSAGE = "Si un compte existe avec cette adresse, un email vous sera envoyé.";
const CANONICAL_SITE_URL = "https://lejapon.ma";
const RECOVERY_PATH = "/espace-voyage/nouveau-mot-de-passe";
const INTERNAL_RECOVERY_BLOCKED_ROLES = new Set([
  "super_admin",
  "admin",
  "manager",
  "sales",
...
const allowedOrigins = () => {
  const configured = (Deno.env.get("CLIENT_RECOVERY_ALLOWED_ORIGINS") || "")
    .split(",")
...
function logStep(step: string, data: Record<string, unknown> = {}) {
  console.log("[client-password-recovery]", { step, function_version: "2026-07-11.1", ...data });
}

function logError(step: string, error: unknown, data: Record<string, unknown> = {}) {
  console.error("[client-password-recovery]", {
    step,
...
async function userHasBlockedInternalRole(admin: any, userId: string) {
  ...
  return roles.some((role: string) => INTERNAL_RECOVERY_BLOCKED_ROLES.has(role));
}
...
async function fetchEmailTemplate(admin: any, key: string, language = "fr") {
  ...
  if (byKey.error && !["42703", "PGRST204", "PGRST205"].includes(String(byKey.error.code ?? ""))) {
    logError("email_template_lookup", byKey.error, { code: byKey.error.code });
  }
  return null;
}
...
function defaultEmailHtml(recoveryLink: string, clientName = "voyageur") {
  return `
    <div style="margin:0;padding:0;background:#f6f3ef;font-family:Arial,Helvetica,sans-serif;color:#171412">
      <div style="max-width:640px;margin:0 auto;padding:28px 14px">
        <div style="padding:0 0 16px;text-align:center">
          <div style="font-size:26px;font-weight:800;color:#E21B2D;letter-spacing:.01em">LeJapon.ma</div>
          <div style="margin-top:4px;font-size:12px;color:#766f68">Moroccan Express Travel & Events</div>
        </div>
        <div style="background:#ffffff;border-radius:18px;border:1px solid #eadfd7;padding:28px;box-shadow:0 8px 24px rgba(0,0,0,.05)">
          <h1 style="margin:0 0 16px;font-size:23px;line-height:1.3;color:#171412">Créez ou réinitialisez votre mot de passe</h1>
          <p style="font-size:15px;line-height:1.7;margin:0 0 14px;color:#3a3531">Bonjour ${escapeHtml(clientName)},</p>
          <p style="font-size:15px;line-height:1.7;margin:0 0 20px;color:#3a3531">
            Vous pouvez créer ou réinitialiser le mot de passe de votre espace voyage LeJapon.ma avec le bouton ci-dessous.
          </p>
          ...
          <p style="font-size:13px;line-height:1.6;margin:0;color:#6b625c">
            ...
            <a href="${escapeHtml(recoveryLink)}" style="color:#E21B2D;word-break:break-all">${escapeHtml(recoveryLink)}</a>
          </p>
          <p style="font-size:13px;line-height:1.6;margin:20px 0 0;color:#6b625c">
            Si vous n'avez pas demandé cet accès, vous pouvez ignorer cet email.
          </p>
        </div>
        <p style="margin:18px 0 0;text-align:center;color:#8a8178;font-size:12px">LeJapon.ma — Espace voyage sécurisé</p>
      </div>
    </div>
  `;
}
...
async function renderRecoveryEmail(admin: any, variables: Record<string, unknown>) {
  const fallback = {
    subject: "Créez ou réinitialisez votre mot de passe LeJapon.ma",
    html: defaultEmailHtml(String(variables.recovery_link), String(variables.client_name || "voyageur")),
    text: `Bonjour ${variables.client_name || "voyageur"},\n\nCréez ou réinitialisez votre mot de passe LeJapon.ma : ${variables.recovery_link}\n\nL'équipe LeJapon.ma`,
  };

  const template = await fetchEmailTemplate(admin, "client_password_recovery");
...
async function findClientName(admin: any, email: string) {
  const { data, error } = await admin
    .from("clients")
    ...
    .maybeSingle();
  if (error) {
    logError("client_lookup", error, { code: error.code });
    return "voyageur";
  }
  const firstName = String(data?.full_name ?? "").trim().split(/\s+/)[0];
  return firstName || "voyageur";
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!email || !isEmail(email)) {
      return json({ ok: false, message: "Adresse email invalide." }, 400, responseCorsHeaders);
    }
    ...
    logStep("request_received", { domain: emailDomain(email) });
    ...
    const blockedInternal = await userHasBlockedInternalRole(admin, authUser.id);
    if (blockedInternal) {
      logStep("recovery_blocked_internal_user", { domain: emailDomain(email), user_id: authUser.id });
      return genericSuccess(responseCorsHeaders);
    }

    const generated = await admin.auth.admin.generateLink({ type: "recovery", email });
    if (generated.error) {
      logError("generate_link_failed", generated.error, { domain: emailDomain(email) });
      return genericSuccess(responseCorsHeaders);
    }

    const tokenHash = generated.data?.properties?.hashed_token;
    if (!tokenHash) {
      logError("missing_hashed_token", "generateLink returned no hashed_token", { domain: emailDomain(email) });
      return genericSuccess(responseCorsHeaders);
    }

    const recoveryLink = `${CANONICAL_SITE_URL}${RECOVERY_PATH}?token_hash=${encodeURIComponent(tokenHash)}&type=recovery`;
    const clientName = await findClientName(admin, email);
    const emailPayload = await renderRecoveryEmail(admin, {
      client_name: clientName,
      recovery_link: recoveryLink,
      login_link: `${CANONICAL_SITE_URL}/espace-voyage/login`,
    });
    ...
    logStep("email_sent", { domain: emailDomain(email) });
    return genericSuccess(responseCorsHeaders);
  } catch (error) {
