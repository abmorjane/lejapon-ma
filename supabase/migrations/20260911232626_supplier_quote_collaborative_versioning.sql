...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(p_rows->'hotels','[]'::jsonb)) loop
    ...
    values(row_id,q.id,coalesce((item->>'sort_order')::int,0),nullif(item->>'city',''),nullif(item->>'hotel_name',''),nullif(item->>'check_in','')::date,nullif(item->>'check_out','')::date,greatest(0,coalesce((item->>'nights')::int,0)),nullif(item->>'room_type',''),greatest(0,coalesce((item->>'rooms_count')::int,(item->>'room_count')::int,0)),greatest(0,coalesce((item->>'room_count')::int,(item->>'rooms_count')::int,0)),greatest(0,coalesce((item->>'unit_price_jpy')::numeric,(item->>'price_per_room_per_night_jpy')::numeric,0)),greatest(0,coalesce((item->>'price_per_room_per_night_jpy')::numeric,(item->>'unit_price_jpy')::numeric,0)),greatest(0,coalesce((item->>'subtotal_jpy')::numeric,0)),nullif(item->>'comment',''),nullif(item->>'assigned_to',''),coalesce(nullif(item->>'status',''),'todo'),statement_timestamp())
  ...
  for item in select value from jsonb_array_elements(coalesce(p_rows->'transport','[]'::jsonb)) loop
    ...
    values(row_id,q.id,coalesce((item->>'sort_order')::int,0),nullif(item->>'service_date','')::date,nullif(item->>'day_number','')::int,nullif(item->>'city_route',''),nullif(item->>'transport_type',''),nullif(item->>'description',''),greatest(0,coalesce((item->>'quantity')::numeric,0)),greatest(0,coalesce((item->>'unit_price_jpy')::numeric,0)),greatest(0,coalesce((item->>'subtotal_jpy')::numeric,0)),nullif(item->>'comment',''),nullif(item->>'assigned_to',''),coalesce(nullif(item->>'status',''),'todo'),statement_timestamp())
  ...
  for item in select value from jsonb_array_elements(coalesce(p_rows->'activities','[]'::jsonb)) loop
    ...
    values(row_id,q.id,coalesce((item->>'sort_order')::int,0),nullif(item->>'service_date','')::date,nullif(item->>'day_number','')::int,nullif(item->>'activity_name',''),greatest(0,coalesce((item->>'participant_count')::numeric,(item->>'quantity')::numeric,0)),greatest(0,coalesce((item->>'quantity')::numeric,(item->>'participant_count')::numeric,0)),greatest(0,coalesce((item->>'unit_price_jpy')::numeric,0)),greatest(0,coalesce((item->>'subtotal_jpy')::numeric,0)),coalesce((item->>'optional')::boolean,false),nullif(item->>'comment',''),nullif(item->>'assigned_to',''),coalesce(nullif(item->>'status',''),'todo'),statement_timestamp())
  ...
  for item in select value from jsonb_array_elements(coalesce(p_rows->'other','[]'::jsonb)) loop
    ...
    values(row_id,q.id,coalesce((item->>'sort_order')::int,0),nullif(item->>'label',''),greatest(0,coalesce((item->>'quantity')::numeric,0)),greatest(0,coalesce((item->>'unit_price_jpy')::numeric,0)),greatest(0,coalesce((item->>'subtotal_jpy')::numeric,0)),nullif(item->>'comment',''),nullif(item->>'assigned_to',''),coalesce(nullif(item->>'status',''),'todo'),statement_timestamp())
